...
#define SC_LAN_LED_BLINK_HZ	(1 * HZ / 10)

unsigned long lan_last_blink[8] = {0, };

static inline int sc_check_vport(int vport)
{
	if (vport < 2 || vport > 5)
		return -1;
	/* vport 2 ~ 5 valid for LAN port: eth1 ~ eth4 */
	return 0;
}

static inline int sc_get_lan_led_id(int vport, int speed)
{
	int lan_port = 5 - vport;
	return (speed == SPEED_1000MBIT) ? LAN_LED_ID_START+1+(lan_port*2) : 
		LAN_LED_ID_START+(lan_port*2); 
}

static void sc_turn_off_led(int vport)
{
	int lan_port = 5 - vport;

	if (sc_led_off_hook) {
		sc_led_off_hook(LAN_LED_ID_START+1+(lan_port*2));
		sc_led_off_hook(LAN_LED_ID_START+(lan_port*2));
	}
}
...
#ifdef _SC_BUILD_
	    /* handle LAN LED for link up event */
	    {
		    int vport = LOGICAL_PORT_TO_VPORT(port);
		    if (sc_led_on_hook && sc_check_vport(vport) == 0)
			    sc_led_on_hook(sc_get_lan_led_id(vport, mib->ulIfSpeed));
	    }
#endif
...
#ifdef _SC_BUILD_
	    /* handle LAN LED for link down event */
	    {
		    int vport = LOGICAL_PORT_TO_VPORT(port);
		    if (sc_check_vport(vport) == 0)
			    sc_turn_off_led(vport);
	    }
#endif
    ...
    do {
        ...
        if (!is_chained && IS_SKBUFF_PTR(pNBuff))
        {
            ...
            if (skb->protocol == htons(ETH_P_ARP))
            {
                ...
                param.mark = SKBMARK_SET_Q_PRIO(param.mark, MAX_PRIORITY_VALUE);
            }
        }   
#endif
...
#ifdef _SC_BUILD_
	/* handle LAN led blinking behavior. */
	{ 
		struct net_device *pNetDev = NULL;
		int vport = LOGICAL_PORT_TO_VPORT(phy_port_id);

		if (sc_check_vport(vport) == 0 && (pNetDev = vnet_dev[vport]) != NULL)
		{
			IOCTL_MIB_INFO *mib  = &((BcmEnet_devctrl *)netdev_priv(pNetDev))->MibInfo;
#if 0
			if (net_ratelimit()) {
				struct net_device *dev = vnet_dev[0];
				BcmEnet_devctrl *priv = (BcmEnet_devctrl *)netdev_priv(dev);
				printk("%s:%d devname=%s, phy_port_id=%d, vport=%d, linkstatus=%08X, speed=%s\n", __func__, __LINE__, skb->dev->name, phy_port_id, LOGICAL_PORT_TO_VPORT(phy_port_id), priv->linkState, (mib->ulIfSpeed == SPEED_1000MBIT) ? "1Gbps" : "10/100Mbps");
			}
#endif
			if (sc_led_blink_data_hook && time_after(jiffies, lan_last_blink[vport] + SC_LAN_LED_BLINK_HZ)) {
				sc_led_blink_data_hook(sc_get_lan_led_id(vport, mib->ulIfSpeed), 1, 10, 10);
				lan_last_blink[vport] = jiffies;
			}
		}
	}
#endif
