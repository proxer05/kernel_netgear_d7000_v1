...
#elif defined(CONFIG_BCM963138) || defined(CONFIG_BCM963381) || defined(CONFIG_BCM963148) || defined(CONFIG_BCM960333)
...
{
#if   !defined(CONFIG_BCM96838) && !defined(CONFIG_BCM963138) && !defined(CONFIG_BCM963381) &&!defined(CONFIG_BCM963148) && !defined(CONFIG_BCM960333)
    unsigned long flags;
...
int kerSysGetGpioDir(unsigned short bpGpio)
{
    ...
    return bcm_gpio_get_dir(bpGpio);
#endif
}

void kerSysSetGpioStateDirectly(unsigned short bpGpio, GPIO_STATE_t state)
{
    unsigned long flags;

    spin_lock_irqsave(&bcm_gpio_spinlock, flags);
    bcm_gpio_set_data(bpGpio, state);
    spin_unlock_irqrestore(&bcm_gpio_spinlock, flags);
}
void kerSysSetGpioPinMux(unsigned short bpGpio)
{
    unsigned long flags;

    spin_lock_irqsave(&bcm_gpio_spinlock, flags);
    bcm_set_pinmux(bpGpio, PINMUX_GPIO);
    spin_unlock_irqrestore(&bcm_gpio_spinlock, flags);
}
#endif
...
#ifdef _SC_BUILD_
EXPORT_SYMBOL(kerSysSetGpioStateDirectly);
EXPORT_SYMBOL(kerSysSetGpioPinMux);
