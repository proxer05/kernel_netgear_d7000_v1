...
static int __init brcm_board_init( void )
{
 ...
 /*disable bcm's control to reset button, for we need to control the button */
#if !defined(_SC_BUILD_)
...
/* To meet NTGR router spec. rev13. MAC address assignment.
 * 1st: LAN MAC & 2.4G MAC
 * 2nd: WAN MAC
 * 3rd: 5G MAC
 * as we have WAN2, although spec does not define it, 
 * we gonna set a different 4th MAC for it.
 */ 
...
struct SNTGRMACRule {
	...
	{0x776C0000, 0xFFFFFFFF, 0}, /* wl0: 2.4G */
	{0xA0001000, 0xFFFFFFFF, 1}, /* eth0.1: WAN */
	{0xE0001000, 0xFFFFFFFF, 1}, /* atm0.1: WAN */
	{0xF0001000, 0xFFFFFFFF, 1}, /* ptm0.1: WAN */
	{0x776C0001, 0xFFFFFFFF, 2}, /* wl1: 5G */
	{0xA0002000, 0xFFFFFFFF, 3}, /* eth0.2: WAN2 */
	{0xE0002000, 0xFFFFFFFF, 3}, /* atm0.2: WAN2 */
	{0xF0002000, 0xFFFFFFFF, 3}, /* ptm0.2: WAN2 */
};

static void sc_initMacInfo(void)
{
	int i, max = sizeof(g_NtgrMacRule) / sizeof(g_NtgrMacRule[0]);
	PMAC_ADDR_INFO pMai = NULL;

	for (i=1; i<max; i++) { /*i=0 does not need to init, LAN always use it.*/
		if(g_NtgrMacRule[i].idx < g_pMacInfo->ulNumMacAddrs) {
			pMai = g_pMacInfo->MacAddrs + g_NtgrMacRule[i].idx;
			pMai->ulId = g_NtgrMacRule[i].ulId;
			pMai->chInUse = 1;
		}
	}
}

static int sc_getMacIdx(unsigned long ulId, unsigned long *macIdx)
{
	int i, max = sizeof(g_NtgrMacRule) / sizeof(g_NtgrMacRule[0]);
	int ret = -1;

	for (i=0; i<max; i++) {
		if ((g_NtgrMacRule[i].ulId & g_NtgrMacRule[i].ulIdMask)
				== (ulId & g_NtgrMacRule[i].ulIdMask)) {
			*macIdx = g_NtgrMacRule[i].idx;
			ret = 0;
			break;
		}
	}
...
static int sc_checkMacIdx(unsigned long macIdx)
{
	int i, max = sizeof(g_NtgrMacRule) / sizeof(g_NtgrMacRule[0]);
	int ret = -1;

	for (i=0; i<max; i++) {
		if ((g_NtgrMacRule[i].idx == macIdx)) {
			ret = 0;
			break;
		}
	}
...
#endif	
#ifdef _SC_BUILD_
    /* check if the MAC address is defined by NTGR */
    if (num_addresses == 1 && sc_getMacIdx(ulId, &i) == 0) 
    {
	    getNthMacAddr( pucaMacAddr, i );
	    pMai = g_pMacInfo->MacAddrs + i;
            pMai->ulId = ulId;
	    pMai->chInUse = 1;
	    pMaiFreeId = NULL;
	    nRet = 0;
...
#ifdef _SC_BUILD_
		    && (num_addresses != 1 || sc_checkMacIdx(ulIdx) != 0) /* keep static MAC addresses. */
#endif
    )
    {
...
{
#ifdef _SC_BUILD_
#define ADSL_GREEN_LED_ID     50
    if(ledName == kLedAdsl)
	{
		switch (ledState)
		{
			case kLedStateOff:
			case kLedStateDSLFail:
			case kLedStateFail:
				if(sc_led_off_hook)
					sc_led_off_hook(ADSL_GREEN_LED_ID);
				break;

			case kLedStateOn:
				if(sc_led_on_hook)
					sc_led_on_hook(ADSL_GREEN_LED_ID);
				break;
			...
			case kLedStateRedSlowBlinkContinues:
				...
				if(sc_led_blink_hook)
					sc_led_blink_hook(ADSL_GREEN_LED_ID, 120, 50, 50);
				break;
			case kLedStateRedFastBlinkContinues:
				if(sc_led_off_hook)
					sc_led_off_hook(ADSL_GREEN_LED_ID);
				if(sc_led_blink_hook)
					sc_led_blink_hook(ADSL_GREEN_LED_ID, 150, 25, 25);
				break;

			default:
				break;
		}
		printk("ADSL led state change to <%d>\n", ledState);
		return;
	}
#endif
    if (g_ledInitialized)
