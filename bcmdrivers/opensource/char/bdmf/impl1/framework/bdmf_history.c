 ...
 */
#ifdef _SC_BUILD_
/* When CPU full loading, like two USB ftp get/send, 100% usage,
 * and then lan-> wan / lan->wlan performance running, the DUT will 
 * crash, porting Sercomm patch for it.
 */
int bug_times = 0; 
#define BDMF_HISTORY_HANDLE_POINT(point) \
    do { \
        ...
        if ((point & bdmf_hist_point_end)) \
        { \
             if((hist.level <= 0) && (bug_times == 0 )) \
             { printk("oliver_debug : kernel bug found\n"); bug_times++;} \
             --hist.level; \
             if (hist.level) \
                 return; \
        } \
