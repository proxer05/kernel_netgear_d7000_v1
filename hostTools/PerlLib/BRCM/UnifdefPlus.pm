...
sub parseCondition {
    ...
    foreach (@defineKeys) {
        my $defineKey = $_;
        $condition =~ s/\bdefined\s*\(\s*$defineKey\s*\)/$TRUE/g;
        $condition =~ s/\bdefined\s*$defineKey$(WS)/$TRUE/g;
    ...
    foreach (@undefineKeys) {
        my $undefineKey = $_;
        $condition =~ s/\bdefined\s*\(\s*$undefineKey\s*\)/$FALSE/g;
        $condition =~ s/\bdefined\s*$undefineKey\b/$FALSE/g;
    }
...
"\n$dbgStr Simplifying expression $string1 ($currentOpPrec) (level=$level)\n"
    ...
    if ( $string1 =~ /^($WS)\((.*)$/s ) {
        ...
        if ( $braceExpr =~ /^($WS)\w+($WS)$/s ) {
            $operand1 = $braceExpr;
            $ws_bo1   = $ws_bb . $1;
            $ws_ao1   = $2;
        }
        elsif ( $braceExpr =~ /^($WS)\w+\s*\([^()]*\)($WS)$/s ) {
        ...
        elsif ( $braceExpr =~ /^($WS)defined\s+\w+($WS)$/s ) {
    ...
    #test for not operator:
        ...
        ( $notOperand, $remainder ) =
          ...
          or return;
        if ( $notOperand =~ /^($WS)(($FALSE)|(0))($WS)$/s ) {
            $operand1 = "$TRUE$5";
        }
        elsif ( $notOperand =~ /^($WS)(($TRUE)|((?:0x)?\d+l?))($WS)$/s ) {
            $operand1 = "$FALSE$5";
        }
        else {
...
# TBD: create isTrue and isFalse functions here, such that isTrue(1) = isTrue(" $TRUE") = isTrue("TRUE")
                    if (
                        ( $operator eq "&&" )
                        && (   ( $operand1 =~ /^($WS)$FALSE($WS)$/s )
                            || ( $operand2 =~ /^($WS)$FALSE($WS)$/s ) )
                      )
                    {
                        print $OUTFILE
...
"$dbgStr end while loop operand1=.$operand1., remainder=.$remainder.\n\n"
    ...
    #return ( $ws_bo1 . $operand1, $remainder );
    return ( $operand1, $remainder );
}
...
sub parseIf {
    ...
    if ( $expression =~ /\s*$TRUE\s*/ ) {
        ...
        while ( $nextExprType =~ /$EXPR->{$lang}->{ELSEIF}/s ) {
            ...
            if ( $nextExpression =~ m/($WS)$TRUE($WS)/ ) {
            ...
            elsif ( $nextExpression =~ /($WS)$FALSE($WS)/ ) {
                ( $nextExprType, $nextConditional ) = $self->parseLines(0);
            }
            else {
...
sub kconfigReadNextEntry {
    ...
    if ($endStr)
    {
        ...
        if ($line =~ /^if\s+(.*?)(\s*(?:\#.*$))?$/) {
            my ($condition, $eolComment) = ($self->kconfigSimplifyExprTop($1), $2);
