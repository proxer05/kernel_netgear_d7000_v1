...
static  S_DPI_CFG       DpiBasicConfig = {DRV_BBH_GPON,1536,0,0,0,0,0,0,0,0};
...
static void f_initialize_dma_sdma(void)
{
    ...
    if(pDpiCfg->wan_bbh!=DRV_BBH_EMAC_4)
    {
        ...
        fi_dma_configure_memory_allocation ( BB_MODULE_DMA ,
                ...
                bbh_rx_dma_data_fifo_base_address [ peripheral_id ] ,
                bbh_rx_dma_chunk_descriptor_fifo_base_address [ peripheral_id ] ,
                BBH_RX_DMA_FIFOS_SIZE_RGMII_LAN_BBH) ;
        /* SDMA */
        fi_dma_configure_memory_allocation ( BB_MODULE_SDMA ,
                DMA_PERIPHERAL_EMAC_4 ,
                bbh_rx_sdma_data_fifo_base_address [ peripheral_id ] ,
    ...
    uint32_t runner_mc_base_addr, int bpm_thresh)
{
    ...
    case DRV_BBH_EMAC_4:
        ...
        break;
    case DRV_BBH_EMAC_5:
        rdd_init_params.wan_physical_port = RDD_WAN_PHYSICAL_PORT_ETH5;
        break;
    case DRV_BBH_GPON:
        rdd_init_params.wan_physical_port = RDD_WAN_PHYSICAL_PORT_GPON;
        break;
    case DRV_BBH_EPON:
        rdd_init_params.wan_physical_port = RDD_WAN_PHYSICAL_PORT_EPON;
        break;
...
#else
    rdd_init_params.ddr_runner_tables_ptr = ddr_runner_base_ptr + (uint32_t)(bpm_defenitions[bpm_thresh].num_bpm * 2048);
#endif
...
{
    uint32_t rc = DPI_RC_OK;
    uint32_t rnrFreq = DEFAULT_RUNNER_FREQ;
    DRV_BBH_PORT_INDEX macIter;
    int bpm_thresh = DRV_BPM_GLOBAL_THRESHOLD_7_5K; /* CFE default */
...
#else
    switch (pCfg->runner_tm_size)
    {
    case 15:
        bpm_thresh = DRV_BPM_GLOBAL_THRESHOLD_5K;
        break;
    case 20:
        bpm_thresh = DRV_BPM_GLOBAL_THRESHOLD_7_5K;
        break;
    case 35:
        bpm_thresh = DRV_BPM_GLOBAL_THRESHOLD_15K;
        break;
    default:
        err("%s %s Allocated TM base memory size not supported [15/20/35] " \
            "(%d)\n", __FILE__, __FUNCTION__, pCfg->runner_tm_size);
        return DPI_RC_ERROR;
    }

    err("nvram TM %dMB: bpm buffers to allocate: %d\n",
        pCfg->runner_tm_size, bpm_defenitions[bpm_thresh].num_bpm);
#endif

    if (f_validate_ddr_address((uint32_t)pCfg->runner_tm_base_addr) != 1)
    {
    ...
    if (f_validate_ddr_address((uint32_t)pCfg->runner_mc_base_addr) != 1)
