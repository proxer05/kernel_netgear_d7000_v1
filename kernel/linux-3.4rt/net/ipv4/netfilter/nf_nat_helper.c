...
#ifdef CONFIG_CNAPT
	if (hooks && 
		hooks->pubip_related(master_ct(ct), range.min_ip))
	{
		/*
 		 * we are in expect() function, NAT extension is not yet available,
 		 * but hooks->out()/cnapt_out() need cnapt which is based on the extension.
 		 * that's why we need to add it here.
 		 */
		struct nf_conn_nat *nat = nfct_nat(ct);
		if (likely(!nat)) {
