...
ipt_dnat_target(struct sk_buff *skb, const struct xt_action_param *par)
{
	...
	struct nf_nat_ipv4_multi_range_compat new_mr;
	int cnapt_act = 0;
	__be32 privip, pubip;
	...
	if (ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum == IPPROTO_TCP ||
		...
		if (get_info && 
			...
			printk("big privip %d   pubip  %d\n",privip,pubip);
			printk("privport %d   pubport  %d\n",privport,pubport);
			/* This must be fake. */
			if (pubip == 0)
			{
				//bypass = 1;
			...
			//if (!bypass) {			    
			if (!cnapt_act) {
				if (mr->range[0].flags & NF_NAT_RANGE_PROTO_SPECIFIED) {
					if (ntohs(mr->range[0].min.tcp.port) != ntohs(mr->range[0].max.tcp.port)) {
			 			//bypass = ((privport != pubport) || (ntohs(mr->range[0].min.tcp.port) > privport)
			 			//	|| (ntohs(mr->range[0].max.tcp.port) < privport));			 		   			 	
			 			...
			 			//bypass = (privport != ntohs(mr->range[0].min.tcp.port));		
			 		if (privport != pubport)
						cnapt_act = 1;
					else
						cnapt_act = 2;	 			
		...
		/*if (bypass)
		{
		    printk("by pass is not 0, drop it\n");
			return XT_CONTINUE;
		}*/
...
#endif
#endif
#if defined(_SC_BUILD_) && defined(CONFIG_SPI_FIRWALL)
...
#if defined(_SC_BUILD_) && defined(CONFIG_CNAPT)
	if (cnapt_act == 2) {
