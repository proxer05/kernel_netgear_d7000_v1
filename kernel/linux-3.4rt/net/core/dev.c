...
#endif
#if defined(_SC_BUILD_) && defined(CONFIG_SC_TRAFFIC_METER)
#include <sc/tm_kernel.h>
#endif
...
#if defined(CONFIG_SC_IPV6_BRIDGE)
...
static int skbFree_thread_func(void *thread_data)
{
    ...
    update_list:
        ...
        local_bh_disable();
        while(free_list && budget) // CS4289983
        {
            skb = free_list;
...
#if defined(_SC_BUILD_) && defined(CONFIG_SC_TRAFFIC_METER)
{
	typeof(sc_tm_check_limited_hook) tm_check_hook;
	typeof(sc_tm_inc_tx_hook) tm_tx_hook;

	tm_check_hook = rcu_dereference(sc_tm_check_limited_hook);
	if(tm_check_hook && tm_check_hook(skb->dev))
	{
		if (net_ratelimit())
			printk(KERN_INFO "TM limit reached: drop Tx packet!\n");
        kfree_skb(skb);
		rc = NET_XMIT_DROP;		
		return rc;
	}
	tm_tx_hook = rcu_dereference(sc_tm_inc_tx_hook);
	if (tm_tx_hook)
		tm_tx_hook(skb->dev, skb);
}
#endif
...
#endif
#ifdef _SC_BUILD_
#define SC_INTERNET_LED		20
	if (sc_led_blink_data_hook && (strncmp(skb->dev->name, "eth0", 4) == 0 
		|| strncmp(skb->dev->name, "atm", 3) == 0
		|| strncmp(skb->dev->name, "ptm", 3) == 0)) { 
		if (time_after(jiffies, wan_led_last_blink + (1*HZ/1000))) {
			sc_led_blink_data_hook(SC_INTERNET_LED, 1, 10, 10 );
...
#if defined(_SC_BUILD_) && defined(CONFIG_SC_TRAFFIC_METER)
{
	...
	{
		if (net_ratelimit())
			printk(KERN_INFO "TM limited: drop Rx packet!\n");        
		kfree_skb(skb);
		return NET_RX_DROP;
	}
}
