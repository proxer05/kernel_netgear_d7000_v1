...
#ifdef _SC_BUILD_
	/*
	* for ipv6 logo pass,spec case 23&26 ,we
	* should forward a multicast package,
	* but its type is neither
	* TCP nor UDP protocol.
	*/
	if(ipv6_hdr(skb)->nexthdr == IPPROTO_UDP || ipv6_hdr(skb)->nexthdr == IPPROTO_ICMPV6)
#else
