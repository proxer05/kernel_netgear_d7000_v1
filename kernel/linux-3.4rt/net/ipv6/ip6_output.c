...
#if defined(_SC_BUILD_) && defined(CONFIG_SC_TRAFFIC_METER)
{
	typeof(sc_tm_inc_rx_hook) tm_rx_hook;
	tm_rx_hook = rcu_dereference(sc_tm_inc_rx_hook);
	if (tm_rx_hook)
		tm_rx_hook(skb->dev, skb);
}
#endif
...
int ip6_forward(struct sk_buff *skb)
{
	...
	if (isDeprecatedAddr(&hdr->saddr, skb->dev)) {
		icmpv6_send(skb, ICMPV6_DEST_UNREACH, ICMPV6_SADDR_FAIL_INGRESS_EGRESS, 0);
		goto error;
	}
#endif
	mtu = dst_mtu(dst);
