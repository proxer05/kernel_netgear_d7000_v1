...
static void dev_forward_change(struct inet6_dev *idev)
{
	...
	list_for_each_entry(ifa, &idev->addr_list, if_list) {
		if (ifa->flags&IFA_F_TENTATIVE)
			continue;
#ifdef _SC_BUILD_
		/* Fix bug: sometimes unregistering DSL interface would failed */
...
static void addrconf_rs_timer(unsigned long data)
{
	...
	if (ifp->probes++ < idev->cnf.rtr_solicits) {
		...
		if( g_trigger_rs == 1 && strcmp(ifp->idev->dev->name, send_rs_interface) == 0 ) {
			printk(KERN_DEBUG "%s: send IPv6 routers solicitation force\n", ifp->idev->dev->name);
			g_trigger_rs = 0;
			ifp->probes = 1;
...
static void addrconf_dad_completed(struct inet6_ifaddr *ifp)
{
	...
	/* send router rs on wan interface always. We will monitor it later*/ 
		|| ( (!strncmp(dev->name, "nas1", 4) || !strncmp(dev->name, "ppp6", 4) || 
	 		!strncmp(dev->name, "eth4", 4)) &&
	    		(dev->flags&IFF_LOOPBACK) == 0 &&
	    		(ipv6_addr_type(&ifp->addr) & IPV6_ADDR_LINKLOCAL) )
#endif	    
	    ){
