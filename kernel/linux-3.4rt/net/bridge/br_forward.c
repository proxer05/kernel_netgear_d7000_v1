...
#include "../../../../../../Source/shared/if_name.h"
#endif
...
wifi_iso_listen_in_kernel wifi_iso_listen_forward_cb = NULL;
int (*dns_hijack_hook)(const struct sk_buff *skb) = NULL;
...
#endif
{
...
#ifdef _SC_BUILD_
	char *ifbr_indev_name;
    /* Add for support wifi isolation - Boom Zhou */
    if(wifi_iso_listen_forward_cb)
    {
        if(wifi_iso_listen_forward_cb((struct sk_buff *)skb))
        {
            return 0;
        }
    }
	if(dns_hijack_hook)
	{
		ifbr_indev_name = skb?(skb->dev?skb->dev->name:NULL):NULL;
		if(ifbr_indev_name)
		{
			if (skb->protocol == htons(ETH_P_IP))
			{
				 if(strstr(ifbr_indev_name, LAN_PHY_IFNAME1) || strstr(ifbr_indev_name, LAN_PHY_IFNAME2)
					|| strstr(ifbr_indev_name, LAN_PHY_IFNAME3) || strstr(ifbr_indev_name, LAN_PHY_IFNAME4) 
					|| strstr(ifbr_indev_name, WLAN_PHY_IFNAME) || strstr(ifbr_indev_name, WLAN_5G_PHY_IFNAME) 
					|| strstr(ifbr_indev_name, WLAN_GUEST_IFNAME) || strstr(ifbr_indev_name, WLAN_5G_GUEST_IFNAME)
					)
					{  	
						if(dns_hijack_hook(skb) != 0) 
						{
							return 0;
						}
					}
